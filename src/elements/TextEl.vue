...
const baseStyle = computed<CSSProperties>(() => {
  ...
  return {
    width: '100%',
    height: '100%',
    padding: paddingValue(s) ?? (s.padding ?? 0) + 'px',
...
function setCaretOffset(root: HTMLElement, offset: number): void {
  ...
  const walk = (n: Node): void => {
    ...
    if (n.nodeType === Node.TEXT_NODE) {
      ...
      if (el.tagName === 'BR') {
        if (remaining === 0) {
          place(el.parentNode!, Array.from(el.parentNode!.childNodes).indexOf(el))
          return
        }
        remaining -= 1
      }
...
<template>
  <div :style="{ width: '100%', height: '100%', position: 'relative' }">
    ...
    <div
      v-else
      ref="viewRef"
      :style="{ ...baseStyle, whiteSpace: 'pre-wrap', cursor: 'move', userSelect: 'none' }"
    >
      {{ element.content }}
    </div>
